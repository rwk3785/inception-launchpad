...
export default function AgentCatalogPage() {
  ...
  const [activeMaturities, setActiveMaturities] = useState<Set<Maturity>>(
    new Set()
  );
  ...
  const filtered = useMemo(() => {
    return agents.filter((agent) => {
      ...
      const matchesQuery =
        !q ||
        agent.business_name.toLowerCase().includes(q) ||
        agent.description.toLowerCase().includes(q);

      const matchesDomain =
        activeDomains.size === 0 ||
        agent.domain.some((d) => activeDomains.has(d));

      const matchesMaturity =
        activeMaturities.size === 0 || activeMaturities.has(agent.maturity);
  ...
  const hasFilters =
    query !== '' || activeDomains.size > 0 || activeMaturities.size > 0;
      ...
      <div className="bg-navy text-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
          ...
          <p className="text-white/70">
            Discover and run AI agents designed for brokerage inception work.
          </p>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-8 space-y-4">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide ml-3 mr-1">
              Maturity:
            </span>
            ...
            {hasFilters && (
              <button
                ...
                className="ml-2 text-xs text-gray-400 underline hover:text-gray-600 transition-colors"
              >
                Clear all
              </button>
            )}
